...
public class ItemOneRing extends ItemBase implements IBauble, IWarpingGear, IRunicArmor {
    ...
    private int getItemWarpLevel(ItemStack stack, boolean increment) {
        ...
        if (stack.hasTagCompound()) {
            if (stack.getTagCompound().hasKey(NBT_TAG_WARP)) {
                ...
                if (increment) stack.getTagCompound().setInteger(NBT_TAG_WARP, ++tWarpLevel);
            }
        } else {
    ...
    public void onWornTick(ItemStack stack, EntityLivingBase player) {
        ...
        if (forgeTag.hasKey(NBT_TAG_MIND_CORRUPTION)) corruption = forgeTag.getInteger(NBT_TAG_MIND_CORRUPTION);
        else forgeTag.setInteger(NBT_TAG_MIND_CORRUPTION, 0);
        ...
        if (!player.worldObj.isRemote) {
            ...
            if (corruption == 0) {
                ...
                ((EntityPlayer) player).addChatMessage(
                        new ChatComponentText(
                                EMTTextHelper.PURPLE + "You have worn the Ring. Your soul has now been temporarily "
            ...
            if (corruption >= 60) { // worn more than 1 minute
    ...
    public boolean canUnequip(ItemStack stack, EntityLivingBase player) {
        // So, how this was written, you basically could not unequip the ring for 600 ticks after you equipped it, but
